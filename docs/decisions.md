...
- 2026-09-25 — One shared `UserMenu` for the sidebar footer and the new top-bar avatar (owner request): larger avatar, full-strength name (DropdownMenuLabel muted it), roomier items via `userMenuItemClass`, including the auth slice's sign-out item (`className` prop).
