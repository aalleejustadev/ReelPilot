...
- 2026-09-25 — Security review (better-auth-security-best-practices checklist): secret, CSRF, trusted origins, DB-backed rate limits and cookie defaults already correct. Enabled `account.encryptOAuthTokens` (Google/GitHub tokens AES-256-GCM at rest). Deferred to deployment (host-specific): HTTPS baseURL, proxy IP headers, HSTS, CSP with nonces.
- 2026-09-25 — Baseline security headers on every response (X-Frame-Options DENY, nosniff, strict-origin-when-cross-origin referrer, Permissions-Policy denying camera/mic/geolocation/display-capture — M2 screen recording must allow display-capture and microphone for self); `poweredByHeader: false`.
- 2026-09-25 — `npm audit`: 4 high findings, all inside Prisma CLI / Better Auth sub-dependencies (deepmerge-ts via @prisma/config; mysql2, unused with Postgres). npm's fix was a Prisma 6 downgrade; used `overrides` instead (mysql2 ^3.23.1, deepmerge-ts ^8.0.0). 0 vulnerabilities; prisma validate/generate/migrate status verified.
- 2026-09-25 — Dead-code pass (knip via npx, not installed): removed unused `checkbox` component and exports/types not used outside their slice; `better-call` added as a dev dependency (test fixture imported it without listing it). Kept on purpose: shadcn component APIs in `src/shared/ui`, `plans.ts` pricing config (M6), `requireWorkspaceAccess` (entry point for M1+).
- 2026-09-25 — M0 (Foundation) complete; completion record with evidence per Definition-of-Done item in build plan §14.2.
